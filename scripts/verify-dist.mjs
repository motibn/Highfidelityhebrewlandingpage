/**
 * נכשל אם אחרי vite build חסרים קבצים קריטיים לפריסת SPA ב־RunCloud/Apache.
 */
import fs from 'node:fs';
import path from 'node:path';
import { fileURLToPath } from 'node:url';

const root = path.join(path.dirname(fileURLToPath(import.meta.url)), '..');
const dist = path.join(root, 'dist');

const required = ['index.html', '.htaccess'];

let ok = true;
for (const name of required) {
  const file = path.join(dist, name);
  if (!fs.existsSync(file)) {
    console.error(`verify-dist: חסר הקובץ ${path.relative(root, file)}`);
    ok = false;
  }
}

const assetsDir = path.join(dist, 'assets');
if (!fs.existsSync(assetsDir)) {
  console.error('verify-dist: חסרה תיקיית dist/assets');
  ok = false;
}

const thankYouIndex = path.join(dist, 'thank-you', 'index.html');
if (!fs.existsSync(thankYouIndex)) {
  console.error(`verify-dist: חסר ${path.relative(root, thankYouIndex)} (נדרש ל־NGINX סטטי בלי SPA rewrite)`);
  ok = false;
}

const hiTechIndex = path.join(dist, 'hi-tech', 'index.html');
if (!fs.existsSync(hiTechIndex)) {
  console.error(`verify-dist: חסר ${path.relative(root, hiTechIndex)} (נדרש ל־/hi-tech/)`);
  ok = false;
}

const hiTechThankYouIndex = path.join(dist, 'hi-tech', 'thank-you', 'index.html');
if (!fs.existsSync(hiTechThankYouIndex)) {
  console.error(`verify-dist: חסר ${path.relative(root, hiTechThankYouIndex)} (נדרש ל־/hi-tech/thank-you/)`);
  ok = false;
}

const hiTechJobsIndex = path.join(dist, 'hi-tech', 'jobs', 'index.html');
if (!fs.existsSync(hiTechJobsIndex)) {
  console.error(`verify-dist: חסר ${path.relative(root, hiTechJobsIndex)} (נדרש ל־/hi-tech/jobs/)`);
  ok = false;
}

const k8musicIndex = path.join(dist, 'k8music', 'index.html');
if (!fs.existsSync(k8musicIndex)) {
  console.error(`verify-dist: חסר ${path.relative(root, k8musicIndex)} (נדרש ל־/k8music/)`);
  ok = false;
}

const k8musicThankYouIndex = path.join(dist, 'k8music', 'thank-you', 'index.html');
if (!fs.existsSync(k8musicThankYouIndex)) {
  console.error(`verify-dist: חסר ${path.relative(root, k8musicThankYouIndex)} (נדרש ל־/k8music/thank-you/)`);
  ok = false;
}

const adminLoginIndex = path.join(dist, 'admin', 'login', 'index.html');
if (!fs.existsSync(adminLoginIndex)) {
  console.error(`verify-dist: חסר ${path.relative(root, adminLoginIndex)} (נדרש ל־/admin/login/)`);
  ok = false;
}

const adminJobsIndex = path.join(dist, 'admin', 'jobs', 'index.html');
if (!fs.existsSync(adminJobsIndex)) {
  console.error(`verify-dist: חסר ${path.relative(root, adminJobsIndex)} (נדרש ל־/admin/jobs/)`);
  ok = false;
}

if (!ok) {
  process.exit(1);
}

console.log('verify-dist: dist תקין (index.html, thank-you/, hi-tech/, hi-tech/jobs/, admin/, k8music/, .htaccess, assets/)');
